...
while len(explored) < len(room_graph):  # while all the rooms hav not been explored
    ...
    # otherwise if the current room has no unused exits, pick one and check it off
        ...
        backtrack.pop()  # remove it from the backtracking path
        traversal_path.append(prevRoom)  # add previous room to traversal path
        player.travel(prevRoom)  # go back to previous room
